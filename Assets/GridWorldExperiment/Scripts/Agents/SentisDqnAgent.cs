...
{
    public class SentisDqnAgent : MonoBehaviour
    {
        ...
        [SerializeField] private float stepIntervalSeconds = 0.12f;
        ...
        {
            if (manager == null || stateEncoder == null)
            {
                return;
            }
            ...
            if (!TryPredictAction(out var action))
            {
                return;
            }
        ...
        public bool TryPredictAction(out GridAction action)
        {
            ...
            var inputTensor = CreateInputTensor(stateEncoder.EncodeFlat(), manager.GridSize);
            if (inputTensor == null)
            {
                return false;
            }
            ...
            try
            {
                ...
                var index = ArgMax(lastQValues);
                action = (GridAction)Mathf.Clamp(index, 0, 3);
                return true;
            }
            catch (Exception exception)
            {
                Debug.LogWarning($"GridWorld ONNX inference failed: {DescribeException(exception)}", this);
                DisposeWorker();
        ...
        private bool TryInitialize()
        {
            if (modelAsset == null)
            {
                Debug.LogWarning("SentisDqnAgent has no model asset assigned.", this);
                return false;
            }
            ...
            {
                Debug.LogWarning("Failed to load GridWorld ONNX model asset.", this);
                return false;
            }
            ...
            var ok = scheduleMethod != null && peekOutputMethod != null && tensorShapeType != null;
            if (verboseLogging)
            {
                Debug.Log(
                    $"[SentisDqnAgent] initialized={ok}, tensor={tensorType}, tensorShape={tensorShapeType}, "
                    + $"schedule={scheduleMethod}, peek={peekOutputMethod}, download={downloadMethod}",
                    this);
            }
        ...
        private static MethodInfo FindScheduleMethod(Type worker, Type tensor)
        {
            return worker.GetMethods().FirstOrDefault(method =>
            {
                ...
                var parameters = method.GetParameters();
                return parameters.Length == 1 && parameters[0].ParameterType == tensor;
            });
        }
        ...
        private object CreateInputTensor(float[] values, int gridSize)
        {
            var shape = CreateTensorShape(gridSize);
            if (shape == null)
            {
                Debug.LogWarning("Failed to create GridWorld input tensor shape.", this);
                return null;
            }

            values = EnsureTensorBufferLength(shape, values);
        ...
        private object CreateTensorShape(int gridSize)
        {
            var constructor = tensorShapeType.GetConstructor(new[] { typeof(int), typeof(int), typeof(int), typeof(int) });
            if (constructor != null)
            {
                return constructor.Invoke(new object[] { 1, StateEncoder.ChannelCount, gridSize, gridSize });
            }

            constructor = tensorShapeType.GetConstructor(new[] { typeof(int[]) });
            return constructor?.Invoke(new object[] { new[] { 1, StateEncoder.ChannelCount, gridSize, gridSize } });
        }

        private float[] EnsureTensorBufferLength(object tensorShape, float[] values)
        {
            var requiredLength = values.Length;
            var lengthProperty = tensorShape.GetType().GetProperty("length", BindingFlags.Public | BindingFlags.Instance)
                ?? tensorShape.GetType().GetProperty("Length", BindingFlags.Public | BindingFlags.Instance);
        ...
        private void OnGUI()
        {
            GUILayout.BeginArea(new Rect(12, 138, 360, 130), GUI.skin.box);
            GUILayout.Label($"Sentis DQN action: {lastAction}");
